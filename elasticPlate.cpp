...
elasticPlate::elasticPlate(double m_YoungM, double m_density, double m_thickness, 
		double m_Possion, double m_dt, int m_nv, double m_delta, double m_height)
{
	YoungM = m_YoungM;
	density = m_density;
...
void elasticPlate::setupGeometry()
{
    ...
    nv_temp_1 = nv;
    double deltaTheta = thetaSpace / ( nv_temp_1 );
    double barRadius = 0.1;

    for (int i = 0; i < nv_temp_1; i++)
    {
    	Vector2d xCurrent;

    	xCurrent(0) = barRadius * cos(-deltaTheta * (i) + M_PI / 2.00001);
    	xCurrent(1) = barRadius * sin(-deltaTheta * (i) + M_PI / 2.00001);

    	v_nodes.push_back(xCurrent);
    }

    double deltaL = 0.5 * barRadius * M_PI / (nv - 1);

    double curveRadius = 0.3;
    double offset = sqrt(curveRadius * curveRadius - barRadius * barRadius);
    double angleOffset = M_PI / 2 - asin(barRadius / curveRadius);
    nv_temp_2 = (M_PI / 2 - angleOffset) * curveRadius / deltaL;
    double deltaTheta2 = (M_PI / 2 - angleOffset) / ( nv_temp_2 );

    for (int i = 0; i < nv_temp_2; i++)
    {
    	Vector2d xCurrent;

    	xCurrent(0) = curveRadius * cos(deltaTheta2 * (i) + angleOffset);
    	xCurrent(1) = curveRadius * sin(deltaTheta2 * (i) + angleOffset) - offset;
...
void elasticPlate::computeEdge()
{
	...
	for (int i = 0; i < edge.size(); i++)
	{
		...
		//cout << m_edgeElement.refLength << endl;
