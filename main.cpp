...
void display(void)
{
	while ( myWorld.simulationRunning() > 0)
	{
		...
		for (int i = 0; i < NPTS; i++)
		{
			...
			glVertex3f(xCurrent_1(0), xCurrent_1(1) - 0.4, 0.0);
			glVertex3f(xCurrent_2(0), xCurrent_2(1) - 0.4, 0.0);
		}
		glEnd();
		...
		for (int i = myWorld.getNv() - 1; i < myWorld.getNv(); i++)
		{
			...
			glVertex3f(xCurrent(0), xCurrent(1) - 0.4, 0.0);
		}
		glEnd();
